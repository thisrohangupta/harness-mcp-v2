  ...
  resources: [
    {
      ...
      executeActions: {
        run: {
          ...
          responseExtractor: ngExtract,
          actionDescription: "Execute/run a pipeline. RECOMMENDED: first check harness_get(resource_type='runtime_input_template', resource_id='PIPELINE_ID') to see required inputs. For CI pipelines with codebase: pass {branch: 'main'}, {tag: 'v1.0'}, {pr_number: '42'}, or {commit_sha: 'abc123'} in inputs — build type is auto-inferred. For simple variables: pass key-value pairs in inputs (e.g. {env: 'prod', replicas: '3'}) — auto-resolved. For complex pipelines with template inputs: use input_set_ids to reference a saved input set. List available sets with harness_list(resource_type='input_set', filters={pipeline_id: '...'}).",
          bodySchema: {
            description: "Runtime inputs for pipeline execution. For CI pipelines with codebase: pass {branch: 'main'}, {tag: 'v1.0'}, {pr_number: '42'}, or {commit_sha: 'abc123'} in inputs — the build type is auto-inferred. For simple variables: pass key-value pairs like {env: 'prod', replicas: '3'}, auto-resolved. For complex pipelines with template inputs: use input_set_ids. You can combine both: input_set_ids for the base config + inputs for overrides. Check runtime_input_template first.",
            fields: [
              { name: "inputs", type: "yaml", required: false, description: "Key-value pairs auto-resolved to full YAML. For CI codebase: {branch: 'main'} or {tag: 'v1.0'} or {pr_number: '42'} or {commit_sha: 'abc123'} — build type auto-inferred. For variables: {env: 'prod', replicas: '3'}. Can combine codebase + variables: {branch: 'main', env: 'prod'}." },
